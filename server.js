...
if (PIPE && PIPE.indexOf('pipe') !== -1) {
  ...
  http.createServer = function() {
    ...
    srv.listen = function(portOrPipe, hostnameOrCb) {
      const p = typeof portOrPipe === 'number' || !isNaN(Number(portOrPipe))
      if (p) {
        process.stderr.write('> Redirecting listen(:' + portOrPipe + ') -> pipe\n')
        // When port is numeric, redirect to the iisnode pipe
  ...
  process.stderr.write('> iisnode pipe: ' + PIPE + '\n')
}
